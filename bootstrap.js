...
function wrappedExpress(...args){
  ...
  app.get('/',(req,res)=>{
    ...
    html=html.replace('</body>','<script src="/slider-live.js"></script><script src="/hero-live.js"></script><script src="/content-live.js"></script></body>');
    res.type('html').send(html);
  });
  app.get('/admin',(req,res)=>{
    const file=path.join(__dirname,'public','admin.html');
    let html=fs.readFileSync(file,'utf8');
    html=html.replace('</body>','<script src="/admin-content.js"></script></body>');
    res.type('html').send(html);
  });
  return app;
}
